...
namespace SoliditySHA3Miner.Utils
{
    public static class Numerics
    {
        ...
        public static void AddressStringToByte20Array(string addressString, ref byte[] address)
        {
            var bytePosition = -1;
            var addressLength = Miner.MinerBase.ADDRESS_LENGTH * 2;
            var addressLowerCase = addressString.ToLowerInvariant();
        ...
        public static byte[] AddressStringToByte20Array(string addressString)
        {
            var addressLength = Miner.MinerBase.ADDRESS_LENGTH * 2;
            addressString = (addressString ?? string.Empty).Replace("0x", string.Empty);

            if (addressString.Length != addressLength)
                throw new InvalidOperationException(string.Format("Invalid address length of {0}.", addressString.Length));

            var address = (byte[])Array.CreateInstance(typeof(byte), Miner.MinerBase.ADDRESS_LENGTH);
            AddressStringToByte20Array(addressString, ref address);
